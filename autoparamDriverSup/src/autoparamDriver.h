...
public:
    ...
    explicit Driver(const char *portName, int interfaceMask, int interruptMask, int asynFlags, int autoConnect, int priority, int stackSize);
...
protected:
    ...
    asynStatus doCallbacksArray(Reason const &reason, Array<T> &value,
                                    int alarmStatus = epicsAlarmNone, int alarmSeverity = epicsSevNone) {
    ...
    // Tole je pa wrapper v asyn stilu. Če hočemo tak stil, je treba pač naštancat te.
    asynStatus doCallbacksInt32Array(Reason const &reason, Array<epicsInt32> &value,
                                     int alarmStatus = epicsAlarmNone, int alarmSeverity = epicsSevNone) {
    ...
    // Od tule naprej so stvari sicer public zaradi interfacea, ampak se derived driverja ne tičejo
 public:

    asynStatus drvUserCreate(asynUser *pasynUser, const char *reason, const char **, size_t *);
    ...
    asynStatus readInt32Array(asynUser *pasynUser, epicsInt32 *value, size_t maxSize, size_t *size) {
...
private:
    ...
    template <typename T>
    asynStatus setParamDispatch(int index, T value);

    template <typename T>
    asynStatus writeScalar(asynUser *pasynUser, T value);

    template <typename T>
    asynStatus readScalar(asynUser *pasynUser, T *value);

    template <typename T>
    asynStatus readArray(asynUser *pasynUser, T *value, size_t maxSize, size_t *size);

    template <typename T>
    std::map<std::string, Handlers<T> >& getHandlerMap();
    ...
    std::map<std::string, Handlers<Array<epicsInt16 > > > m_int16ArrayHandlerMap;
    // in tako dalje
};
    ...
    asynStatus Driver::writeScalar(asynUser *pasynUser, T value) {
        ...
        try {
            result = getHandlerMap<T>().at(reason->function()).writeHandler(*reason, value);
    ...
    template <typename T>
    asynStatus Driver::readArray(asynUser *pasynUser, T *value, size_t maxSize, size_t *size) {
        Reason *reason = reasonFromUser(pasynUser);
        if (!reason) {
            return asynError;
        }
        ...
        try {
            result = getHandlerMap<Array<T> >().at(reason->function()).readHandler(*reason);
        } catch (std::out_of_range const&) {
            // No handler
        }
    ...
    template <>
    asynStatus Driver::doCallbacksArrayDispatch<epicsInt32>(int index, Array<epicsInt32> &value) {
        return asynPortDriver::doCallbacksInt32Array(value.data(), value.size(), index, 0);
    }

        template <>
        asynStatus Driver::doCallbacksArrayDispatch<epicsInt64>(int index, Array<epicsInt64> &value) {
            return asynPortDriver::doCallbacksInt64Array(value.data(), value.size(), index, 0);
        }
    ...
    template <>
    std::map<std::string, Handlers<epicsInt32> >& Driver::getHandlerMap<epicsInt32>() {
