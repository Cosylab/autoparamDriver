...
Reason::Reason(Reason const &other) {
    *this = other;
}
...
Driver::Driver(const char *portName, int interfaceMask, int interruptMask, int asynFlags, int autoConnect, int priority, int stackSize) :
    asynPortDriver(portName, 1, interfaceMask, interruptMask, asynFlags, autoConnect, priority, stackSize)
 {
    // TODO
}
...
asynStatus Driver::drvUserCreate(asynUser *pasynUser, const char *reason, const char **, size_t *) {
    ...
    try {
        ...
        asynPrint(this->pasynUserSelf, ASYN_TRACE_ERROR,
                  ...
                  driverName, portName, parsed.function().c_str());
        return asynError;
    }
